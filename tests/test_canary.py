...
def test_canary_review_is_a_failure(monkeypatch):
    """`review` means the pipeline declined to auto-post; the canary must not
    call that healthy. It did under real mode while bug #7 put it there
    routinely, which blinded it to exactly the failure it exists to catch."""
