...
`pytest tests/ -v` — deterministic extraction, real Postgres, real threads for the concurrency
cases, ~4s for 86 tests. `conftest.py`'s session-scoped fixture forces the deterministic backend
regardless of the ambient shell environment, so the suite stays hermetic even when
`EXTRACTION_MODE=real` is set (e.g. after sourcing `.env` for something else).
...
- `test_gates.py`, `test_extraction_funnel.py`, `test_business_dedupe.py` — the five quality gates, via a programmable deterministic model (`injected_total`, `swapped_roles`, `refusal`, `context_overflow` scenarios)
