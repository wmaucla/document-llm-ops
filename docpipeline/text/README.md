...
| `ocr_engine.py` | The OCR engine abstraction — `DeterministicOcrEngine` (default, deterministic, keyed by `(doc_id, page_no)`) and `TesseractOcrEngine` (opt-in via `OCR_ENGINE=tesseract`) | `stages/ocr_shard_3.py`, and `fixtures/generate_fixtures.py` (registers ground-truth OCR text for fixtures) |
