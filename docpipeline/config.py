...
# attempt_log is append-only diagnostic history with no duplicate elsewhere, so
# it needs retention (docpipeline.reconciliation.prune). outbox does not -- the
# relay deletes on delivery ack, so it is bounded by the backlog.
