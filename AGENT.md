...
mechanics that aren't obvious from reading the code once.

**This file is current-state only.** A bug that gets fixed moves to
[HISTORY.md](HISTORY.md)'s closed bug register; what stays here is the one still-open bug and
the constraints the closed ones left behind, because those are rules about the code as it is
now, not stories about how it got that way.

## Open bug

One entry, still open. Everything else this repo has chased is fixed and now
lives in [HISTORY.md](HISTORY.md)'s "Closed bug register" — the constraints those
bugs left behind are under "Standing constraints" below, so nothing actionable
moved out of this file.

Numbers are cited from code (`config.py` cites bug #3) and are never reused:

| # | where | one line |
|---|---|---|
| 1 | **below** | liveness probe killed slow-but-healthy extraction pods |
| 2 | HISTORY.md | ollama silently fell back to CPU |
| 3 | HISTORY.md | four timeouts for one operation disagreed; now derived from one budget |
| 4 | HISTORY.md | outbox relay marked undelivered messages published |
| 5 | HISTORY.md | attempt-capped documents reached `failed` with an empty `last_error` |
| 6 | HISTORY.md | `summary-k8s` failed the play on a healthy in-flight pipeline |
| 7 | HISTORY.md | `arithmetic` gate passed on a *missing* total, disabling tier escalation |
| 8 | HISTORY.md | `summarize.py`/`wait_for_drain.py` leaked connections |
| 9 | HISTORY.md | deterministic-OCR registry unreachable across pods |
| 10 | HISTORY.md | `wait_for_drain` could declare victory before all documents were ingested |
...
Rules the closed bugs left behind. Each one is a change that looks reasonable and
is not; the full reasoning is in [HISTORY.md](HISTORY.md)'s closed bug register.

- **Never enable auto-sync on `mlops-llm-serving`** (bug #2). `site.yml` patches the
  live ollama Deployment (`OLLAMA_KEEP_ALIVE=-1`, `OLLAMA_MAX_LOADED_MODELS=2`, 8Gi)
  while deliberately not editing the sibling repo's manifest, so that Application sits
  permanently `OutOfSync`/`Healthy`. **The drift is the fix.** Auto-sync or selfHeal
  would silently roll it back; `gpu-watchdog` would catch the CPU fallback, but nothing
  watches the memory limit, so the starvation half would return unobserved. Move the
  patches into the sibling's manifest first if you ever want it auto-synced.
- **Do not split `EXTRACT_STUCK_THRESHOLD_SECONDS` by state** (bug #3). Shortening it
  for `extract_pending` on the reasoning that nobody holds those documents is wrong and
  reintroduces the bug on the `_pending` side. Measured under a forced-CPU backlog:
  `lag == pending + running` exactly — those messages are queued behind slow inference,
  not stranded, so there is nothing for a faster redrive to rescue. The trigger to
  revisit is `lag ≈ 0` while documents sit in `extract_pending`; discard any sample
  taken mid-rebalance, where `rpk` reports lag 0 indistinguishably from a drained topic.
- **`restartCount` is the ground truth for probe kills** (bug #1). Do not use
  `kubectl get events --field-selector reason=Killing` — it matches routine
  `Normal`/"Stopping container" rollout terminations and has produced a false alarm.
- **Don't conclude "wedged" from silence** (bug #1). Every extraction replica shares one
  Ollama pod, and a document needing repair rounds issues one full call per round, so
  10+ minutes with long silent gaps is legitimate. Require either a blocked query in
  `pg_stat_activity`/`pg_locks`, or a wait past the canary's own 900s SLO.
- **A high `review` count means inference degraded, not unusual documents** (bug #7).
  Established by a failed prediction: the same generator gave 13/13 `review` under
  forced CPU and 3/3 `complete` on a healthy GPU. Investigate it as a GPU problem.
- **An `applies_to` predicate is not the only evasion surface** (bug #7). `arithmetic`
  was hardened so a model could not disable it by omitting `line_items`, then passed
  anyway when the model omitted the very field being checked. A gate must never return
  `pass` on missing input.
- **"Self-consistent, so nothing breaks" is a claim about one deployment topology**
  (bug #9). Any file written by one process and read by another becomes a shared-storage
